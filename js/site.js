(() => {
  ...
  if(list && window.RESEARCH_OUTPUTS){
    ...
    function row(x,idx){const doi=x.doi?`<a href="${x.doiUrl}" target="_blank">DOI ${esc(x.doi)} ↗</a>`:`<span>DOI not listed in current CV</span>`;const fam=x.family?`<span class="output-family">${esc(x.family)}</span>`:'';return `<article class="output-row" data-cat="${esc(x.category)}" data-search="${esc((x.title+' '+x.detail+' '+(x.doi||'')+' '+(x.family||'')).toLowerCase())}"><div class="output-num">${String(idx+1).padStart(3,'0')}</div><div class="output-main"><small>${esc(x.type)} · ${esc(x.year)}</small><h3>${esc(x.title)}</h3><p>${esc(x.detail)}</p></div><div class="output-meta">${doi}${fam}</div></article>`}
